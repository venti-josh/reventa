...
class CRUDOrgAllowedDomain(
    CRUDBase[OrgAllowedDomain, OrgAllowedDomainCreate, OrgAllowedDomainUpdate]
):
    async def get_by_domain(
        self, db: AsyncSession, *, domain: str
    ) -> OrgAllowedDomain | None:
        result = await db.execute(
            select(OrgAllowedDomain).where(OrgAllowedDomain.domain == domain)
        )
        return result.scalar_one_or_none()

    async def get_by_org_id(
        self, db: AsyncSession, *, org_id: UUID
    ) -> list[OrgAllowedDomain]:
        result = await db.execute(
            select(OrgAllowedDomain).where(OrgAllowedDomain.org_id == org_id)
        )
