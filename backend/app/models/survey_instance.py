...
class SurveyInstance(Base):
    ...
    launched_at: Mapped[TIMESTAMP | None] = mapped_column(TIMESTAMP(timezone=True), nullable=True)

    # Relationships
    organization: Mapped[Organization] = relationship(Organization, back_populates="survey_instances")
    ...
    links: Mapped[list["Link"]] = relationship("Link", back_populates="survey_instance", cascade="all, delete-orphan")
